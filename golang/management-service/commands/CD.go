...
func (c *CD) RunCommand(args []string, id *ConsoleId) (string, *ConsoleId) {
	if len(args) == 0 {
		if id.Parent() != nil {
			return "", id.Parent()
		}
	...
	if c.sm != nil {
		return c.handleServiceManager(c, args, id)
	} else {
		return c.handleService(c, args, id)
	}
}

func (c *CD) handleServiceManager(command Command, args []string, id *ConsoleId) (string, *ConsoleId) {
	serviceID := args[0]
	if len(args) > 1 {
...
func (c *CD) handleService(command Command, args []string, id *ConsoleId) (string, *ConsoleId) {
